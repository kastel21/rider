"""Driver weekly M&E export columns (40 columns, comma-separated national template).

See ``operations/fixtures/mande_driver_report_reference.csv`` for the header row.
"""
...
from .me_report_schema import MEReportColumn

DRIVER_ME_REPORT_COLUMNS: tuple[MEReportColumn, ...] = (
    MEReportColumn(
        "transport_kind",
        "Transport kind",
        "neutral",
        "trip.dominant_transport_kind_label",
    ),
    MEReportColumn("driver_name", "Name of Driver ", "basic", "rider.full_name"),
    MEReportColumn("vehicle_reg", "Vehicle Registration Number", "basic", "report.bike_or_car"),
    ...
    MEReportColumn("fuel_allocated", "Fuel allocated to driver this  week", "compare", "trip.fuel_allocated_total"),
    MEReportColumn("fuel_used", "Fuel used by driver this  week", "compare", "trip.fuel_used_total"),
    MEReportColumn("distance", "Distance travelled by driver this  week", "compare", "trip.distance_total"),
    MEReportColumn(
        "days_vehicle_functional",
        "Number of days  vehicle was functional",
        "compare",
        "report.days_vehicle_functional",
    ),
    ...
    MEReportColumn("snp_breakdown", "Vehicle breakdown", "outcome", "vehicle.snp_bike_breakdown"),
    MEReportColumn(
        "snp_service",
        "Vehicle on routine service/maintenance",
        "outcome",
        "vehicle.snp_bike_routine_service",
    ),
    MEReportColumn("snp_no_fuel", "Vehicle had no fuel", "outcome", "vehicle.snp_bike_no_fuel"),
    MEReportColumn("snp_sick", "Driver on sick leave", "outcome", "vehicle.snp_rider_sick_leave"),
    MEReportColumn("snp_annual", "Driver on annual leave", "outcome", "vehicle.snp_rider_annual_leave"),
    MEReportColumn("snp_weather", "Inclement weather", "outcome", "vehicle.snp_inclement_weather"),
    MEReportColumn("snp_accident", "Vehicle  accident damaged", "outcome", "vehicle.snp_bike_accident"),
